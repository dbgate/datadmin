...
namespace DatAdmin
{
    public class ArrayDataRecord : IBedRecord, IBedRecordWriter
    {
        object[] m_values;
        ITableStructure m_structure;
        int m_curReadField = -1;
        int m_curWriteField = -1;
        BedValueHolder m_workingHolder = new BedValueHolder();
        bool m_loadedValue;

        public ArrayDataRecord(ITableStructure structure, object[] values)
        {
            m_values = values;
            m_structure = structure;
        }

        public ArrayDataRecord(ITableStructure structure)
        {
            m_structure = structure;
            m_values = new object[structure.Columns.Count];
        }

        public ArrayDataRecord(IBedRecord record)
        {
        ...
        #region IBedRecordWriter Members
