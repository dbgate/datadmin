...
namespace DatAdmin
{
    public static partial class BedTool
    {
        ...
        public static void SaveToXml(ITableStructure table, IEnumerable records, XmlWriter xw)
        {
            ...
            foreach (IColumnStructure col in table.Columns) ids.Add(XmlTool.NormalizeIdentifier(col.ColumnName));
        ...
        public static IEnumerable<IBedRecord> LoadFromXml(ITableStructure table, XmlReader reader)
        {
            ...
            {
                if (reader.LocalName != "Row") throw new XmlFormatError(String.Format("DAE-00216 Bad xml, expected tag {0}, found {1}", "Row", reader.LocalName));
                reader.Read();
                reader.MoveToContent();
